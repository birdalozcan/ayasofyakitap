...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Siparis Bilgi Formu</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
             <form name="form1" method="post" id="form1"  action="urunler.jsp">
	        ...
	        <sql:query dataSource="${veriKaynagi}" var="sorgu">
	          SELECT * FROM uyeler INNER JOIN siparis ON siparis.uyeid=uyeler.uye_id;
	        </sql:query>
         		...
         		<div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                   ...
                                   <c:forEach var="satir" items="${sorgu.rows}">
                                    <tbody>
                                        <tr class="odd gradeX">
                                         <td><c:out value="${satir.siparis_id}"/></td>
                                            <td><c:out value="${satir.uye_id}"/></td>
                                            <td><c:out value="${satir.uyeadi}"/></td>
                                            <td><c:out value="${satir.uyesoyadi}"/></td>
                                              <td><c:out value="${satir.fatura_adi}"/></td>
                                            <td><c:out value="${satir.tutar}"/></td>
                                            <td><c:out value="${satir.adres}"/></td>
                                            <td class="center"><c:out value="${satir.odemetipi}"/></td>
                                            <td class="center"><c:out value="${satir.siparisdurumu}"/></td>
                                            <td class="center"><c:out value="${satir.olusma_zamani}"/></td>
                                                
                                            <td class="center"><a  href="siparisduzenle.jsp?id=<c:out value="${satir.uye_id}"/>">Düzenle</a> |
                                            				   <a  href="siparissil.jsp?id=<c:out value="${satir.siparis_id}"/>">Sil</a>
                                            </td>
                                        </tr>
                                   </tbody>
                                   </c:forEach>
