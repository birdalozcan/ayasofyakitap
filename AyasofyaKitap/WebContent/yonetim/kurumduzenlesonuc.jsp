...
<body>
    ...
    <div id="wrapper">
    	...
    	<!--  	
	      ...
	      <sql:update dataSource="${veriKaynagi}" var="sorgu">
           
            UPDATE kategoriler SET kategoriadi = ?, kategorisirasi=?, kategoriaktif=?
            WHERE kategori_id='${param.id}';
            
            <sql:param value="${param.kategoriadi}" />
            <sql:param value="${param.kategorisirasi}" />
            <sql:param value="${param.kategoriaktif}" />  
          </sql:update>
    				
    	<c:if test="${sorgu>=1}">
           <font size="5" color='green'> İslem Basarılı</font>
              <a href="kategoriler.jsp">Go Home</a>              
        </c:if>		
    	-->	
    	<%        
     ...
     String  sqlDuzenle = "UPDATE kurumbilgileri "
         + " SET kurum_adi = ?, "
         + " email = ?, "
         + " telefon = ?, "
         + " adres = ?, "
         + " fax = ?, "
         + " aktif = ? "
         + " WHERE kurumbilgileri_id = 1";
     
        
   preparedStatementDuzenle = conDuzenle.prepareStatement(sqlDuzenle);
   
   preparedStatementDuzenle.setString(1, request.getParameter("kurumadi"));
   preparedStatementDuzenle.setString(2, request.getParameter("email"));
   preparedStatementDuzenle.setString(3, request.getParameter("telefon"));
   preparedStatementDuzenle.setString(4, request.getParameter("adres"));
   preparedStatementDuzenle.setString(5, request.getParameter("fax"));
   preparedStatementDuzenle.setString(6, request.getParameter("aktiflik"));
   
   
   
   preparedStatementDuzenle.executeUpdate();
   //System.out.println("Güncelleme Basarılı");
   response.sendRedirect("kurumbilgileri.jsp");
